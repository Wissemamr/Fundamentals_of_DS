...
def save_standardized_data(df : pd.DataFrame, convert_df_to_ndarray : bool = True )-> (pd.DataFrame | np.ndarray):
...
def apply_PCA(unreduced_data: pd.DataFrame) -> np.ndarray:
    stand_data_ndarr = save_standardized_data(unreduced_data)
    if DEBUG : print(f'The standardized data with z-score : {stand_data_ndarr}')
    ...
    if DEBUG : print(f'The sorted eigenvalues: {sorted_eigenvals} \n The sorted eigenvects : {sorted_eigenvects}')
...
if __name__ == "__main__":
    ...
    result = apply_PCA(unreduced_data=df)
